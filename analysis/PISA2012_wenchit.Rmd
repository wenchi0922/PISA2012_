...
-- PISA2012 data
In this analysis, the original dataset is provided by library [@PISA2012lite]
...
assessment is 0 and maximum is 1,000. For reliability, maller states and indigenous students were oversampled in this assessment. 

-- Technique

Plots:
the plots are generated with the [@ggplot] library.
...
In the linear regression test, the p values are all smaller than 0.05 which implies that the store locations are highly related to a student's math performance.
...
From the Tukey test above, telling from the column diff and p adj, some interesting facts can be able to conclude:
...
2. There are significant discrepancy in math performace between City-Village, Large City-Village, Large City-Samll Town, Large City-Town, since the p value all equal to 0. These significant difference is shown in the plotted Tukey graph. Those pairs that are significantly different are those that does not across the 0 value.
...
In the linear regression test, the p values are all smaller than 0.05 which implies that the whether the school is a private on or public one is highly related to a student's math performance.
...
4. The mean score for all Australian students is 495.5, whereas the students from a village is 

 While there has been growth in the number of people from regional and remote areas 
